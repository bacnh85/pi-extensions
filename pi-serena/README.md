...
When a worker request exceeds the configured timeout, the Python bridge process is automatically killed and a fresh worker is started for the next call. Requests are serialized to match the Python bridge's sequential protocol, so a timed-out request should not reject later queued requests. Tools routed through the adapter's shared `callWorkerAction` path (all `serena_*` tools except `serena_status`, `serena_list_tools`, and the `/serena-dashboard` command, which call the worker directly) retry transient worker timeout/restart failures once. If a request is expected to take longer, pass a larger `timeout_ms`; if worker state appears stale, run `/serena-restart`. Exiting Pi should not normally be needed.
