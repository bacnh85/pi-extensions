...
| [`@bacnh85/pi-a2a`](./pi-a2a) | 0.5.0 | A2A Protocol v1.0 bidirectional — Pi distributes tasks to remote agents (Hermes, ADK, LangChain, any A2A peer), exposes itself as an A2A-callable agent, self-declares for local session discovery (file registry + enriched Agent Card + mDNS), shows inbound task activity in the host TUI, and has an interactive config panel. |
