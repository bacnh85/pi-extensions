...
| [`@bacnh85/pi-subagent`](./pi-subagent) | 0.22.0 | Isolated in-process subagents with parallel/chain modes, inspectable threads, git worktree isolation (`sandbox: worktree`), opt-in auto-review, and herdr pane delegation (`runner: "herdr"` + `herdr` control tool) when pi runs inside herdr. |
